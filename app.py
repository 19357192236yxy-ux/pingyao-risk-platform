...
st.set_page_config(
    page_title="平遥古城气候风险量化评估平台 V1.3.2",
    page_icon="🏛️",
    layout="wide",
...
st.markdown(
    ...
    <style>
    .main-header {
        background: linear-gradient(90deg, #1f4e79 0%, #2f6f9f 100%);
        padding: 24px 28px;
        border-radius: 16px;
        color: white;
        margin-bottom: 18px;
...
def show_risk_badge(level: str):
    st.markdown(
        f'<div class="risk-badge" style="background-color:{risk_color(level)};">综合风险等级：{level}</div>',
        unsafe_allow_html=True,
    )
...
def calc_climate_indices(df, date_col, tmax_col, tmin_col, pre_col):
    ...
    pre_raw = pd.to_numeric(data[pre_col], errors="coerce")
    ...
    meta = {
        "起始日期": data[date_col].min(),
        "结束日期": data[date_col].max(),
        "记录条数": len(data),
        "最高气温缺失数": int(tmax.isna().sum()),
        "最低气温缺失数": int(tmin.isna().sum()),
        "降水量缺失数": int(pre_raw.isna().sum()),
    }
...
def make_result_excel(results_df, climate_df=None, subrisk_df=None):
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        if subrisk_df is not None:
            subrisk_df.to_excel(writer, sheet_name="单灾种风险结果", index=False)
...
def generate_diagnosis(result):
    ...
    drivers = []
    if H >= 0.6:
        drivers.append("气候危险性较高，说明高温、强降水、连续降雨或冻融等外部气候压力较明显。")
    if E >= 0.6:
        drivers.append("暴露度较高，说明居民、游客、传统建筑和公共服务设施较为集中。")
    if V >= 0.6:
        drivers.append("脆弱性较高，说明传统建筑、排水条件、街巷空间或生态调节能力存在短板。")
    if AC < 0.5:
        drivers.append("适应力偏弱，说明排水改造、应急避难、预警响应或修缮维护能力仍需提升。")
    if not drivers:
        drivers.append("各风险成分整体处于中低水平，但仍需结合历史事件进行校验。")

    suggestions = [
        "加强连续降雨和强降水条件下的传统建筑巡查与修缮维护。",
        "完善古城排水系统、易涝点治理和雨洪调蓄能力。",
        "优化节假日游客高峰期疏散、预警发布和公共服务保障。",
        "结合老年居民和游客暴露特征，完善高温热浪和低温冻融应急响应。",
    ]

    return (
        f"平遥古城及周边人居空间综合气候风险指数为 **{R:.3f}**，风险等级为 **{level}**。"
        f"从风险结构看，气候危险性为 **{H:.3f}**，暴露度为 **{E:.3f}**，"
        f"脆弱性为 **{V:.3f}**，适应力为 **{AC:.3f}**。\n\n"
        "**主要风险驱动因素：**\n"
...
st.markdown(
    ...
    <div class="main-header">
        <h1>平遥古城气候风险量化评估平台 V1.3.2</h1>
        <p>面向人居环境及基础设施的 H—E—V—AC 综合指数评估原型工具</p>
    </div>
    """,
    unsafe_allow_html=True,
)
...
page = st.sidebar.radio(
    "请选择功能页面",
    ["首页", "数据导入与校验", "气候危险性指标计算", "风险成分赋值与权重", "综合评价结果", "数据导出"],
    index=0,
)

st.sidebar.markdown("---")
st.sidebar.subheader("综合风险权重")
...
st.sidebar.caption(f"当前权重合计：{weight_sum:.2f}")
...
if page == "首页":
    st.markdown(
        ...
        <b>数据安全提示：</b>公开演示版本仅用于方法展示，请勿上传涉密、内部或未公开数据。
        真实项目数据建议在本地环境或内部服务器运行。
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    c1.metric("研究对象", "平遥古城")
    c2.metric("评价框架", "H-E-V-AC")
    c3.metric("风险类型", "5类")
    c4.metric("输出成果", "指数+图表+Excel")

    st.markdown("### 平台定位")
    st.write("本平台用于演示平遥古城及周边人居空间在气候变化背景下的风险评估流程。")
    st.markdown("### 评估流程")
    st.markdown(
        """
        1. **数据导入与校验**：上传数据收集表或逐日气候数据，检查字段和缺失值。  
    ...
    st.info("当前 V1.3.2 仍属于原型工具。E、V、AC 仍采用 0—1 分值输入；后续可升级为自动读取 Excel 原始指标并标准化计算。")
    ...
    if uploaded_excel is not None:
        try:
            ...
            preview_df = excel_data[sheet]
            st.dataframe(preview_df, use_container_width=True)
            c1, c2, c3 = st.columns(3)
            c1.metric("工作表数量", len(excel_data))
            c2.metric("当前表行数", preview_df.shape[0])
            c3.metric("当前表列数", preview_df.shape[1])
        except Exception as e:
            st.error(f"读取 Excel 失败：{e}")
    else:
        st.info("如果只是测试平台，也可以先不上传 Excel，直接进入“风险成分赋值与权重”。")

elif page == "气候危险性指标计算":
    st.header("气候危险性指标计算")
    st.write("上传逐日气候数据，至少需要包含：日期、最高气温、最低气温、降水量。")
    ...
    if climate_file is not None:
        try:
            ...
            check_df = pd.DataFrame({
                "字段": [date_col, tmax_col, tmin_col, pre_col],
                "缺失值数量": [
                    climate_df[date_col].isna().sum(),
                    pd.to_numeric(climate_df[tmax_col], errors="coerce").isna().sum(),
                    pd.to_numeric(climate_df[tmin_col], errors="coerce").isna().sum(),
                    pd.to_numeric(climate_df[pre_col], errors="coerce").isna().sum(),
                ],
            })
            ...
            if st.button("计算气候危险性指标", type="primary"):
                ...
                st.session_state["climate_meta"] = meta
                st.success("气候指标计算完成")

            if "climate_result" in st.session_state:
                st.subheader("气候指标计算结果")
                st.dataframe(st.session_state["climate_result"], use_container_width=True)
                meta = st.session_state.get("climate_meta", {})
                if meta:
                    m1, m2, m3 = st.columns(3)
                    m1.metric("记录条数", meta.get("记录条数", "-"))
                    m2.metric("起始日期", str(meta.get("起始日期", ""))[:10])
                    m3.metric("结束日期", str(meta.get("结束日期", ""))[:10])
        except Exception as e:
            st.error(f"气候数据处理失败：{e}")
    else:
        st.info("请上传逐日气候数据。若暂时没有数据，可先进入“风险成分赋值与权重”手动设置 H 值。")

elif page == "风险成分赋值与权重":
    st.header("风险成分赋值与权重设置")
    st.info("单个案例 V1.3.2 暂采用 0—1 分值输入。0 表示最低，1 表示最高。")
    ...
    R = float(wH * H + wE * E + wV * V + wAC * (1 - AC))
    level = risk_level(R)
    ...
    subrisk_df = pd.DataFrame({
        "风险类型": ["高温热浪风险", "暴雨洪涝风险", "连续降雨建筑潮湿风险", "干旱缺水风险", "低温冻融风险"],
        "危险性分值": [H_heat, H_flood, H_rain, H_drought, H_freeze],
    })
    subrisk_df["风险指数_示范"] = subrisk_df["危险性分值"].apply(lambda h: wH * h + wE * E + wV * V + wAC * (1 - AC))
    subrisk_df["风险等级"] = subrisk_df["风险指数_示范"].apply(risk_level)
    st.session_state["subrisk_result"] = subrisk_df

    with col_right:
        st.subheader("即时计算结果")
        st.metric("综合风险指数 R", f"{R:.3f}")
        show_risk_badge(level)
        st.caption("计算公式：R = wH×H + wE×E + wV×V + wAC×(1-AC)")
        st.dataframe(pd.DataFrame({"维度": ["H", "E", "V", "1-AC"], "权重": [wH, wE, wV, wAC]}), use_container_width=True)

elif page == "综合评价结果":
    st.header("综合评价结果")

    if "risk_result" not in st.session_state:
        st.info("请先进入“风险成分赋值与权重”页面完成计算。")
    else:
        result = st.session_state["risk_result"]
        result_df = pd.DataFrame({"项目": list(result.keys()), "结果": list(result.values())})
        subrisk_df = st.session_state.get("subrisk_result")

        c1, c2, c3, c4, c5 = st.columns(5)
        c1.metric("H 气候危险性", f"{result['H 气候危险性']:.3f}")
        c2.metric("E 暴露度", f"{result['E 暴露度']:.3f}")
        c3.metric("V 脆弱性", f"{result['V 脆弱性']:.3f}")
        c4.metric("AC 适应力", f"{result['AC 适应力']:.3f}")
        c5.metric("R 综合风险", f"{result['R 综合气候风险']:.3f}")

        show_risk_badge(result["风险等级"])
        ...
        g1, g2 = st.columns(2)
        with g1:
            st.plotly_chart(plot_bar(chart_df), use_container_width=True)
        with g2:
            st.plotly_chart(plot_radar(chart_df["维度"], chart_df["指数值"]), use_container_width=True)

        if subrisk_df is not None:
            st.subheader("单灾种风险示范结果")
            st.dataframe(subrisk_df, use_container_width=True)
            fig = go.Figure()
            fig.add_trace(go.Bar(
                x=subrisk_df["风险类型"],
                y=subrisk_df["风险指数_示范"],
                text=[f"{v:.3f}" for v in subrisk_df["风险指数_示范"]],
                textposition="outside",
            ))
            fig.update_layout(
                title="单灾种风险指数示范",
        ...
        st.subheader("自动诊断结论")
        st.markdown(generate_diagnosis(result))

elif page == "数据导出":
    st.header("数据导出")

    if "risk_result" not in st.session_state:
        st.info("请先完成风险计算，再导出结果。")
    else:
        result = st.session_state["risk_result"]
        result_df = pd.DataFrame({"项目": list(result.keys()), "结果": list(result.values())})
        climate_df = st.session_state.get("climate_result")
        subrisk_df = st.session_state.get("subrisk_result")
        ...
        st.download_button(
            ...
            data=output,
            file_name="平遥古城气候风险计算结果_V1.3.2.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        )

        st.subheader("可复制文字结论")
        st.markdown(generate_diagnosis(result))
